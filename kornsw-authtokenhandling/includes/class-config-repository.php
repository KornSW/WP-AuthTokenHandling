...
final class KornSW_ATH_Config_Repository {
    ...
    const OPTION_PRIMARY_SOURCE = 'kornsw_ath_primary_source_uid';
    ...
    public static function get_profiles() {
        ...
        foreach ($decoded as $profile) {
            if (is_array($profile) && !empty($profile['TokenSourceUid'])) {
                $result[] = $profile;
            }
        }
        return $result;
    ...
    public static function save_profiles_json($json) {
        ...
        if (!is_array($decoded)) {
            return new WP_Error('invalid_json', 'Die Profilkonfiguration ist kein gültiges JSON-Array.');
        }
        $uids = array();
        foreach ($decoded as $index => $profile) {
            if (!is_array($profile)) {
                return new WP_Error('invalid_profile', 'Profil #' . ($index + 1) . ' ist kein JSON-Objekt.');
            }
            $uid = isset($profile['TokenSourceUid']) ? trim((string) $profile['TokenSourceUid']) : '';
            if ($uid === '') {
                return new WP_Error('missing_uid', 'Profil #' . ($index + 1) . ' hat keine TokenSourceUid.');
            }
            if (isset($uids[$uid])) {
                return new WP_Error('duplicate_uid', 'TokenSourceUid ist doppelt vorhanden: ' . $uid);
            }
            $uids[$uid] = true;
            if (!isset($profile['AuthTokenConfig']) || !is_array($profile['AuthTokenConfig'])) {
                return new WP_Error('missing_config', 'Profil ' . $uid . ' hat keine AuthTokenConfig.');
            }
        }
    ...
    public static function get_profile($uid) {
        foreach (self::get_profiles() as $profile) {
            if (hash_equals((string) $profile['TokenSourceUid'], (string) $uid)) {
                return $profile;
            }
        }
        return null;
    ...
    public static function get_primary_source_uid() {
        return (string) get_option(self::OPTION_PRIMARY_SOURCE, '');
    }
    ...
    public static function create_default_profile() {
        return array(
            ...
            'AuthTokenConfig' => array(
                'DisplayLabel' => 'Authentification',
                'DisplayIconUrl' => '',
                'IssueMode' => 'OAUTH_CIBA_CODEGRAND',
                'RetrieveEndpointUrl' => '',
                'RetrieveEndpointAuthorization' => '',
                'LocalLogonNameToLower' => false,
                'LocalLogonNamePersistation' => 'OPT-IN',
                'LocalLogonNameSyntax' => '',
                'LocalLogonNameInputLabel' => '',
                'LocalLogonPassInputLabel' => '',
                'LocalLogonSaltDisplayLabel' => '',
                'JwtExpMinutes' => 1440,
                'JwtSelfSignKey' => '',
                'JwtSelfSignAlg' => 'HS256',
                'ClientId' => '',
                'ClientSecret' => '',
                'AuthEndpointUrl' => '',
                'AdditionalAuthArgs' => '',
                'AdditionalRetrieveArgs' => '',
                'AuthEndpointRejectsIframe' => true,
                'ValidationMode' => 'OAUTH_INTROSPECTION_ENDPOINT',
                'ValidationOutcomeCacheMins' => 2,
                'JwtValidationKey' => '',
                'ClaimValidationIgnoresCasing' => true,
                'ValidationEndpointUrl' => '',
                'validationEndpointAuthorization' => '',
                'Claims' => array()
            )
        );
    }
