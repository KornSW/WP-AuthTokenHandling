...
final class KornSW_ATH_Generic_Provider extends KornSW_ATH_Provider_Base {
    ...
    private function identity_from_claims($c){$sub=(string)($c['sub']??$c['id']??'');if($sub==='')return new WP_Error('missing_subject','Provider liefert keinen stabilen Subject-Identifier.');$display=(string)($c['name']??$c['display_name']??$c['preferred_username']??$c['login']??'');$given=(string)($c['given_name']??$c['first_name']??'');$family=(string)($c['family_name']??$c['last_name']??'');if(($given===''||$family==='')&&$display!==''){$parts=preg_split('/\s+/',trim($display),2);if($given===''&&!empty($parts[0]))$given=$parts[0];if($family===''&&!empty($parts[1]))$family=$parts[1];}return array('subject'=>$sub,'email'=>(string)($c['email']??''),'given_name'=>$given,'family_name'=>$family,'display_name'=>$display,'claims'=>$c);}
