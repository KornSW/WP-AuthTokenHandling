...
function kswkornswauthtof97d_bootstrap( $plugin_file ) {
	static $instances = [];
	...
	$instance = new KSWKORNSWAUTHTOF97DSelfUpdate( $plugin_file );
...
class KSWKORNSWAUTHTOF97DSelfUpdate {
		...
		public function register_hooks(): void {
			if (
				is_admin()
				&& defined( 'KSWKORNSWAUTHTOF97D_SELF_UPDATE_DIAGNOSTICS' )
				&& KSWKORNSWAUTHTOF97D_SELF_UPDATE_DIAGNOSTICS
			) {
		...
		public function render_update_refresh_notice(): void {
			if (
				...
				|| ! isset( $_GET['kswkornswauthtof97d_update_state_refreshed'] )
				|| '1' !== (string) $_GET['kswkornswauthtof97d_update_state_refreshed']
			) {
				return;
			}
		...
		public function maybe_refresh_wordpress_update_state(): void {
			...
			if ( isset( $_GET['kswkornswauthtof97d_refresh_update_state'] ) ) {
				$requested_plugin = sanitize_text_field(
					wp_unslash( (string) $_GET['kswkornswauthtof97d_refresh_update_state'] )
				);

				if ( $requested_plugin === $this->plugin_basename ) {
					check_admin_referer( 'kswkornswauthtof97d_refresh_update_state' );
					$this->refresh_wordpress_update_state();

					wp_safe_redirect(
						add_query_arg(
							'kswkornswauthtof97d_update_state_refreshed',
							'1',
		...
		public function filter_plugin_row_meta( $links, $plugin_file ) {
			...
			if ( current_user_can( 'update_plugins' ) ) {
				$refresh_url = wp_nonce_url(
					add_query_arg(
						'kswkornswauthtof97d_refresh_update_state',
		...
		public function register_diagnostics_page(): void {
			add_management_page(
				...
				'Self-Update Diagnose (KornSW)',
				'manage_options',
				'kswkornswauthtof97d-self-update-diagnostics',
		...
		public function render_diagnostics_page(): void {
			...
			if ( isset( $_POST['ksw_force_update_check'] ) ) {
				check_admin_referer( 'kswkornswauthtof97d_force_update_check' );
			...
			$runtime_rows = [
				...
				'Bootstrap-Funktion'        => 'kswkornswauthtof97d_bootstrap',
				'Updater-Klasse'            => 'KSWKORNSWAUTHTOF97DSelfUpdate',
			...
			<div class="wrap">
				...
				<form method="post" style="margin: 18px 0;">
					<?php wp_nonce_field( 'kswkornswauthtof97d_force_update_check' ); ?>
